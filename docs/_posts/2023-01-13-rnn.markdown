...
In many introductions to RNN you'll note that the output is usually a single data point. It's either the next character in a char-rnn, or the next data point in a time series one. In this derivation and code we're going to use the more general for so that we could predict any number of data points.
