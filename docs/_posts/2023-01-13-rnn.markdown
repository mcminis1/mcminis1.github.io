...
I added in $$h_p$$ because it will be useful later when computing in the $$\bar W$$ derivative. 

## Optimization

The parameters are initiated randomly. I did a little bit of work to minimize the error with initial params, but keep the matrices well ocnditioned. You can see that in the code where I do SVD or QR decompositions. 

Below are plots depicting the RNN predictions converging from their random initial conditions to full optimization, and in some cases a little further. I randomly choose 4 segments to plot (that are _not_ in the trianing set). The red line is our prediction $$\tilde y$ and the green line is the ground truth. This is the progress over 4000 epochs.
